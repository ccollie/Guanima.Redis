    ...
    public static class FastActivator
    {
        ...
        static class Helper
        {
            ...
            public static string MakeSafeName(Type type)
            {
                ...
                retval.Append('_').Append(_counter++);
        ...
        class MultiArgFactory
        {
            ...
            public object CreateInstance(Type type, Type[] ctorArgs, object[] ctorValues)
            {
                ...
                return GetFactory(type, ctorArgs).CreateInstance(ctorValues);
            }
            ...
            private IFastMultiArgObjectFacory GetFactory(Type type, Type[] ctorArgs)
            {
                ...
                if (!factoryCache.TryGetValue(hcc.CurrentHash, out retval))
                {
                    lock (factoryCache)
                    {
                        if (!factoryCache.TryGetValue(hcc.CurrentHash, out retval))
                        {
                            ...
                            factoryCache.Add(hcc.CurrentHash, retval);
                        }
                    }
                }
