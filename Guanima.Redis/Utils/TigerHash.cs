...
namespace Guanima.Redis.Utils
{
    ...
    public sealed class TigerHash : HashAlgorithm
    {
        ...
        private ulong a;
        private ulong b;
        private readonly ulong[] block = new ulong[8];
        private int _bufferPosition;
        private ulong c;

        private readonly byte[] _internalDataBuffer;
        ...
        public TigerHash()
        {
            _internalDataBuffer = new byte[BLOCKSIZE];
            HashSizeValue = 3*8*8;
        ...
        protected override void HashCore(byte[] array, int ibStart, int cbSize)
        {
            ...
            byte[] buffer = _internalDataBuffer;

            int offset = _bufferPosition;
        ...
        {
            int bufferOffset = _bufferPosition;
            byte[] buffer = _internalDataBuffer;
            ...
            LongToBytes(((ulong) _totalLength) << 3, buffer, BLOCKSIZE - 8);

            ProcessBlock();

            var retval = new byte[24];
        ...
        private unsafe void ProcessBlock()
        {
            ...
            //byte[] buffer = this._internalDataBuffer;
            ...
            Buffer.BlockCopy(_internalDataBuffer, 0, block, 0, BLOCKSIZE);
        ...
        private void RoundABC(ulong x, uint mul)
        {
            ...
            b += T[((cl >> 8) & 0xFF) + 0x300] ^ T[((cl >> 24) & 0xFF) + 0x200] ^ T[((ch >> 8) & 0xFF) + 0x100] ^
                 T[(ch >> 24) & 0xFF];
        ...
        private void RoundBCA(ulong x, uint mul)
        {
            ...
            c += T[((al >> 8) & 0xFF) + 0x300] ^ T[((al >> 24) & 0xFF) + 0x200] ^ T[((ah >> 8) & 0xFF) + 0x100] ^
                 T[(ah >> 24) & 0xFF];
            c *= mul;
        ...
        private void RoundCAB(ulong x, uint mul)
        {
            ...
            a += T[((bl >> 8) & 0xFF) + 0x300] ^ T[((bl >> 24) & 0xFF) + 0x200] ^ T[((bh >> 8) & 0xFF) + 0x100] ^
                 T[(bh >> 24) & 0xFF];
            a *= mul;
